...
impl TransformerTrait for Transformer {
    fn parse(document: &Bytes) -> anyhow::Result<Document> {
        ...
        let root = json
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Root element is not a JSON object"))?;
        ...
        let elements = parse_elements(
            &root
                .get("elements")
                .ok_or_else(|| anyhow::anyhow!("Missing 'elements' field"))?
                .clone(),
        )?;
    ...
    fn generate(document: &Document) -> anyhow::Result<Bytes> {
            ...
            match element {
                ...
                Element::Table { headers, rows } => {
                    let headers_json: Vec<Value> = headers
                        ...
                        .map(|h| {
                            ...
                            h_map.insert(
                                "width".to_string(),
                                Value::Number(
                                    serde_json::Number::from_f64(h.width as f64).unwrap(),
                                ),
                            );
                            Value::Object(h_map)
                        })
                    ...
                    let rows_json: Vec<Value> = rows
                        .iter()
                        .map(|r| {
                            let cells_json: Vec<Value> = r
                                .cells
                                .iter()
                                .map(|c| serialize_element(&c.element))
                                .collect();
                ...
                Element::List { elements, numbered } => {
                    let elements_json: Vec<Value> = elements
                        ...
                        .map(|item| {
                            ...
                            item_map
                                .insert("element".to_string(), serialize_element(&item.element));
                ...
                Element::Image(image_data) => {
                    ...
                    map.insert(
                        "title".to_string(),
                        Value::String(image_data.title().to_string()),
                    );
                    map.insert(
                        "alt".to_string(),
                        Value::String(image_data.alt().to_string()),
                    );
                    map.insert(
                        "image_type".to_string(),
                        Value::String(image_data.image_type().to_string()),
                    );

                    map.insert(
                        "align".to_string(),
                        Value::String(image_data.align().to_string().to_lowercase()),
                    );
        ...
        // Serialize elements
        let elements_json: Vec<Value> = document
            .get_detail()
            .into_iter()
        ...
        doc_map.insert(
            ...
            Value::Number(
                serde_json::Number::from_f64(document.page_format.dimensions().page_width as f64)
                    .unwrap(),
            ),
        );
        doc_map.insert(
            ...
            Value::Number(
                serde_json::Number::from_f64(document.page_format.dimensions().page_height as f64)
                    .unwrap(),
            ),
        );
        doc_map.insert(
            "left_page_indent".to_string(),
            Value::Number(
                serde_json::Number::from_f64(
                    document.page_format.dimensions().page_margin_left as f64,
                )
                .unwrap(),
            ),
        );
        doc_map.insert(
            "right_page_indent".to_string(),
            Value::Number(
                serde_json::Number::from_f64(
                    document.page_format.dimensions().page_margin_right as f64,
                )
                .unwrap(),
            ),
        );
        doc_map.insert(
            "top_page_indent".to_string(),
            Value::Number(
                serde_json::Number::from_f64(
                    document.page_format.dimensions().page_margin_top as f64,
                )
                .unwrap(),
            ),
        );
        doc_map.insert(
            "bottom_page_indent".to_string(),
            Value::Number(
                serde_json::Number::from_f64(
                    document.page_format.dimensions().page_margin_bottom as f64,
                )
                .unwrap(),
            ),
        );
        ...
        let page_header_json: Vec<Value> = document
            .get_page_header()
            .into_iter()
            .map(serialize_element)
            .collect();
        ...
        let page_footer_json: Vec<Value> = document
            .get_page_footer()
            .into_iter()
            .map(serialize_element)
            .collect();
...
fn parse_elements(value: &Value) -> anyhow::Result<Vec<Element>> {
    let array = value
        .as_array()
        .ok_or_else(|| anyhow::anyhow!("'elements' is not an array"))?;
...
fn parse_element(value: &Value) -> anyhow::Result<Element> {
    let obj = value
        .as_object()
        .ok_or_else(|| anyhow::anyhow!("Element is not an object"))?;
    let type_str = obj
        .get("type")
        .and_then(|v| v.as_str())
    ...
    match type_str {
        "Text" => {
            ...
            let size =
                obj.get("size").and_then(|v| v.as_u64()).ok_or_else(|| {
                    anyhow::anyhow!("Text element missing or invalid 'size' field")
                })? as u8;
            Ok(Element::Text { text, size })
        }
        "Header" => {
            let level =
                obj.get("level").and_then(|v| v.as_u64()).ok_or_else(|| {
                    anyhow::anyhow!("Header element missing or invalid 'level' field")
                })? as u8;
            let text = obj
                .get("text")
                .and_then(|v| v.as_str())
        ...
        "Paragraph" => {
            let elements = parse_elements(
                &obj.get("elements")
                    .ok_or_else(|| anyhow::anyhow!("Paragraph missing 'elements' field"))?
                    .clone(),
            )?;
            Ok(Element::Paragraph { elements })
        }
        "Table" => {
            let headers = parse_table_headers(
                &obj.get("headers")
                    .ok_or_else(|| anyhow::anyhow!("Table missing 'headers' field"))?
                    .clone(),
            )?;
            let rows = parse_table_rows(
                &obj.get("rows")
                    .ok_or_else(|| anyhow::anyhow!("Table missing 'rows' field"))?
                    .clone(),
            )?;
        ...
        "List" => {
            let numbered = obj
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!("List element missing or invalid 'numbered' field")
                })?;
            let list_items = obj
                .get("elements")
                .ok_or_else(|| anyhow::anyhow!("List element missing 'elements' field"))?;
            let items_array = list_items
                .as_array()
                .ok_or_else(|| anyhow::anyhow!("List 'elements' is not an array"))?;
        ...
        "Hyperlink" => {
            ...
            let size = obj.get("size").and_then(|v| v.as_u64()).ok_or_else(|| {
                anyhow::anyhow!("Hyperlink element missing or invalid 'size' field")
            })? as u8;
            Ok(Element::Hyperlink {
                title,
                url,
                alt,
                size,
...
fn parse_table_headers(value: &Value) -> anyhow::Result<Vec<TableHeader>> {
    let headers_array = value
        .as_array()
        .ok_or_else(|| anyhow::anyhow!("'headers' is not an array"))?;
    let mut headers = Vec::new();

    for header in headers_array {
        let header_obj = header
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Header is not an object"))?;
        let element = parse_element(
            &header_obj
                .get("element")
                .ok_or_else(|| anyhow::anyhow!("Header missing 'element' field"))?
                .clone(),
        )?;
        let width = header_obj
            .get("width")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| anyhow::anyhow!("Header missing or invalid 'width' field"))?
            as f32;
...
fn parse_table_rows(value: &Value) -> anyhow::Result<Vec<TableRow>> {
    let rows_array = value
        .as_array()
        .ok_or_else(|| anyhow::anyhow!("'rows' is not an array"))?;
    let mut rows = Vec::new();

    for row in rows_array {
        let row_obj = row
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("Row is not an object"))?;
        let cells = row_obj
            .get("cells")
            .ok_or_else(|| anyhow::anyhow!("Row missing 'cells' field"))?;
        let cells_array = cells
            .as_array()
            .ok_or_else(|| anyhow::anyhow!("Row 'cells' is not an array"))?;
...
fn parse_list_item(value: &Value) -> anyhow::Result<ListItem> {
    let obj = value
        .as_object()
        .ok_or_else(|| anyhow::anyhow!("ListItem is not an object"))?;
    let element = parse_element(
        &obj.get("element")
            .ok_or_else(|| anyhow::anyhow!("ListItem missing 'element' field"))?
            .clone(),
    )?;
    Ok(ListItem { element })
}
